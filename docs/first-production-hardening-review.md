...
Apps Script receives POST and GET fallback events, writes raw rows internally, aggregates to Sheets, and publishes a chunked aggregate JSON payload. `Mælaborðsmælingar` loads aðeins samantektargögn JSONP from `api=dashboard` with a 22-second timeout, one automatic retry, section-level safe rendering, and a fast server read path that avoids full workbook setup unless the published payload read fails.
